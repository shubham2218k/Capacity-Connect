...
const AdminRegister = () => {
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'var(--bg-color)', padding: '2rem 1rem' }}>
        <div className="card" style={{ maxWidth: '600px', width: '100%', padding: '3rem', textAlign: 'center' }}>
          ...
          <div style={{ backgroundColor: 'var(--bg-color-alt)', padding: '1.5rem', borderRadius: '8px', textAlign: 'left', marginBottom: '2rem' }}>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', fontSize: '0.9rem', color: 'var(--text-light)', marginBottom: '0.5rem' }}>Trainee Access Key</label>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <input type="text" value={successData.traineeKey} readOnly style={{ flex: 1, padding: '0.75rem', borderRadius: '6px', border: '1px solid var(--border-color)', backgroundColor: 'var(--white)', fontWeight: 600, fontFamily: 'monospace' }} />
                <button type="button" onClick={() => copyToClipboard(successData.traineeKey)} className="btn btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <Copy size={16} /> Copy
                </button>
              </div>
            </div>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.9rem', color: 'var(--text-light)', marginBottom: '0.5rem' }}>Trainer Access Key</label>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <input type="text" value={successData.trainerKey} readOnly style={{ flex: 1, padding: '0.75rem', borderRadius: '6px', border: '1px solid var(--border-color)', backgroundColor: 'var(--white)', fontWeight: 600, fontFamily: 'monospace' }} />
                <button type="button" onClick={() => copyToClipboard(successData.trainerKey)} className="btn btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <Copy size={16} /> Copy
                </button>
              </div>
            </div>
          ...
          <button onClick={() => navigate('/admin/dashboard')} className="btn btn-primary" style={{ padding: '1rem 2rem', fontSize: '1.1rem', width: '100%' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'var(--bg-color)', padding: '2rem 1rem' }}>
      <div className="card" style={{ maxWidth: '900px', width: '100%', padding: '2.5rem' }}>
        ...
        <form onSubmit={handleRegister}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '2rem' }}>
            ...
            <div>
              ...
              <div className="input-group">
                ...
                <select id="organizationType" name="organizationType" value={formData.organizationType} onChange={handleChange} style={{ padding: '0.75rem', borderRadius: '6px', border: '1px solid var(--border-color)', width: '100%' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div className="input-group">
                  ...
                  <input type="email" id="officialEmail" name="officialEmail" value={formData.officialEmail} onChange={handleChange} required />
                </div>
                <div className="input-group">
                  <label htmlFor="officialPhone">Official Phone</label>
                  <input type="tel" id="officialPhone" name="officialPhone" value={formData.officialPhone} onChange={handleChange} />
                </div>
              </div>
          ...
          <div style={{ marginTop: '3rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <button type="submit" disabled={isSubmitting} className="btn btn-primary" style={{ padding: '1rem 3rem', fontSize: '1.1rem', minWidth: '250px', opacity: isSubmitting ? 0.7 : 1 }}>
              {isSubmitting ? 'Registering...' : 'Register Organization'}
            </button>
            <p style={{ marginTop: '1.5rem', fontSize: '0.9rem', color: 'var(--text-light)' }}>
              Already registered? <Link to="/login" style={{ color: 'var(--secondary)', fontWeight: 600 }}>Sign in here</Link>
            </p>
          </div>
        </form>
      </div>
